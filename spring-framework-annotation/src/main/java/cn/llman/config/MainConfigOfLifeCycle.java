 ...
 * ---  这里值得注意的是：Bean的创建、构造、实例化，表达的是同一个概念，即：IOC容器通过反射机制使用该bean的类文件，调用构造函数实例化对象的过程。
 * <p>
 ...
 * ------       init-method：在bean创建(构造、实例化)完毕后调用(单实例bean在容器创建时就会创建，多实例bean在获取对象是才会创建)
 * ------       destroy-method：在容器关闭时调用，单实例的bean随着容器的关闭而销毁；多实例的bean，容器只负责bean的创建(构造)，不负责bean的销毁
 ...
 * ---              在此需要明白的是，bean的实例化和初始化是不一样的，实例化时bean的构造，属性赋值等；初始化是指bean已经创建(构造)了，再做的一些init工作
 ...
 * ---
 * ---  TODO    源码：{@link AbstractAutowireCapableBeanFactory} 的 initializeBean 方法
 ...
 * ---          由此可以看出populateBean依然属于Bean实例化的过程
 * <p>
